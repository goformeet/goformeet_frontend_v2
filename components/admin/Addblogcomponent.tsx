...
async function onSubmit(values: z.infer<typeof blogSchema>) {
  // Extract the first image URL if it's an array
  const image = Array.isArray(values.image) ? values.image[0] : values.image;
  // const apiUrl = `http://localhost:3009/create-blog`;
  const apiUrl = `${process.env.NEXT_PUBLIC_BACKEND_URL}/create-blog`;
  console.log("add blog model is called");
  try {
